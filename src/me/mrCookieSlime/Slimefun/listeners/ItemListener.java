...
public class ItemListener implements Listener {
    ...
    public void onUse(PlayerInteractEvent e) {
		...
		ItemMeta im = item.getItemMeta();
	        for(String lore : im.getLore()) {
	        	if (lore.equals(ChatColor.translateAlternateColorCodes('&', "&bCúp - Rìu - Xẻng"))) {

	            	Material[] BlockAxe = {
	            			Material.WOOD, Material.LOG, Material.LOG_2,
	            			Material.ACACIA_STAIRS, Material.BIRCH_WOOD_STAIRS, Material.DARK_OAK_STAIRS, Material.JUNGLE_WOOD_STAIRS, Material.SPRUCE_WOOD_STAIRS, Material.WOOD_STAIRS,
	            			Material.WOOD_STEP,
	            			Material.CHEST, Material.TRAPPED_CHEST, Material.WORKBENCH, Material.LADDER,
	            			Material.FENCE, Material.FENCE_GATE, Material.ACACIA_FENCE, Material.ACACIA_FENCE_GATE, Material.BIRCH_FENCE, Material.BIRCH_FENCE_GATE,  Material.DARK_OAK_FENCE, Material.DARK_OAK_FENCE_GATE, Material.JUNGLE_FENCE, Material.JUNGLE_FENCE_GATE, Material.SPRUCE_FENCE, Material.SPRUCE_FENCE_GATE,
	            			Material.SIGN, Material.BANNER, Material.WOOD_PLATE, Material.TRAP_DOOR, Material.ACACIA_DOOR, Material.BIRCH_DOOR, Material.DARK_OAK_DOOR, Material.JUNGLE_DOOR, Material.SPRUCE_DOOR, Material.BOOKSHELF, Material.PUMPKIN, Material.MELON_BLOCK, Material.JACK_O_LANTERN
	            	};
	            	Material[] BlockShovel = {
	                		Material.DIRT, Material.GRASS_PATH, Material.MYCEL, Material.GRASS, Material.GRAVEL, Material.SAND, Material.SOUL_SAND, Material.CLAY, Material.SNOW_BLOCK, Material.CONCRETE_POWDER
	                };
	            	item.setType(Material.DIAMOND_PICKAXE);
	            	for (Material axe : BlockAxe) {
	            		if (block == axe) {
	            			item.setType(Material.DIAMOND_AXE);
	            			break;
	            		}
	            	};
                	for (Material shovel : BlockShovel) {
                		if (block == shovel) {
	...
	public void onIgnitionChamberItemMove(InventoryMoveItemEvent e) {
		if (e.getInitiator().getHolder() instanceof Dropper) {
			if (BlockStorage.check(((Dropper) e.getInitiator().getHolder()).getBlock(), "IGNITION_CHAMBER")) {
				e.setCancelled(true);
			}
		}
	}
    ...
    public void onHelicopterHatUse(PlayerToggleSneakEvent e) {
        ...
        if (e.isSneaking() && SlimefunManager.isItemSimiliar(e.getPlayer().getEquipment().getHelmet(), SlimefunItems.HELICOPER_HAT, true)) {
            p.addPotionEffect(new PotionEffect(PotionEffectType.LEVITATION, 1000000, 3));
        } else {
    ...
    public void onPlayerDamage(EntityDamageEvent e) {
        if (e.getEntity() instanceof Player && e.getCause() == EntityDamageEvent.DamageCause.FALL) {
        	if (((Player) e.getEntity()).getEquipment().getHelmet() != null) {
        		if (SlimefunManager.isItemSimiliar(((Player) e.getEntity()).getEquipment().getHelmet(), SlimefunItems.HELICOPER_HAT, true)) {
        			e.setCancelled(true);
        		}
        	}
        }
