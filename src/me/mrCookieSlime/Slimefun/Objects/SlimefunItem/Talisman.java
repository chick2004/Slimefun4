...
public class Talisman extends SlimefunItem {
		...
		if (talisman != null) {
			if (talisman instanceof Talisman) {
				...
				if (SlimefunStartup.chance(100, ((Talisman) talisman).getChance())) {
					...
					else if (e instanceof PlayerExpChangeEvent) p = ((PlayerExpChangeEvent) e).getPlayer();
	...
	public ItemStack upgrade() {
		...
		lore.add("");
		lore.add("&7Chỉ hoạt động khi nằm trong rương Ender");
		for (String line: getItem().getItemMeta().getLore()) {
			lore.add(line);
		}
		ItemStack item = new CustomItem(getItem().getType(), ChatColor.stripColor(getItem().getItemMeta().getDisplayName()).replace("Ngọc hộ mệnh", "&aNgọc hộ mệnh &5Ender").replace("[", "&7[&e").replace("]", "&7]"), getItem().getDurability(), lore.toArray(new String[lore.size()]));
		return item;
	}
